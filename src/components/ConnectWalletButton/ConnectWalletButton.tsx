...
export const ConnectWalletButton: FC = () => {
    ...
    useEffect(() => {
        if (window.ethereum) {
            ...
            // @ts-ignore
            window.ethereum.on('chainChanged', (chainID: string) => setChainID(parseInt(chainID, 16)))
            // @ts-ignore
            window.ethereum.on('accountsChanged', (accounts: Array<string>) => setAccounts(accounts));
            window.ethereum.on('networkChanged', (...args) => console.log('networkChanged', args))
        }
    ...
    if (status === 'initial') {
        return (
            <Button type='button' variant="contained" disabled={false}>Connect Wallet</Button>
        )
    }

    if (status === 'not_installed') {
        return (
            <Button type='button' variant="contained" endIcon={<MetamaskIcon/>} disabled={false} onClick={onClickInstall}>Click here to install MetaMask!</Button>
        )
    }

    if (status === 'onboarding') {
        return (
            <Button type='button' variant="contained" disabled={true}>Onboarding in progress</Button>
        )
    }
    ...
    if (status === 'connected') {
        ...
        if (chainID === Web3.utils.hexToNumber(ETHEREUM_CHAIN.chainId)) {
            return (
                <Button type='button' variant="contained" fullWidth endIcon={<MetamaskIcon/>} disabled={true}>Connected with {truncateEthAddress(accounts[0])}</Button>
            )
        } else {
            return (
                <Button type='button' variant="contained" disabled={false} onClick={handleSetChain}>Change Chain</Button>
            )
        }
