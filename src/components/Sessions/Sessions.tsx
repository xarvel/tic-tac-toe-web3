...
import { List, ListItem, ListItemButton, ListItemText, Radio, ListItemIcon, ListSubheader} from "@mui/material";
...
export const Sessions:FC<SessionsProps> = ({ account, onChangeSessionID, sessionID, contract }) => {
    const [sessions, setSession] = useRecoilState(sessionsAtom);

    const reloadAvailableSessions = useCallback(() => {
        if(account){
            contract.methods.getAvailableSessions(account).call({
                from: account
            }).then((result: string[]) => {
                setSession(result.map((sessionID) => ({
    ...
    useEffect(() => {
        ...
        subscription = contract.events.SessionCreated({})
    ...
    return (
        <div className='sessions'>
            <List subheader={<ListSubheader>Sessions</ListSubheader>}>
                ...
                {
                    sessions.map((session) => <ListItem key={session.sessionID}>
                        <ListItemButton onClick={() => onChangeSessionID(session.sessionID)}>
                            <ListItemIcon>
                                <Radio
                                    edge="start"
