...
import {EmptyBoard, Mark, playgroundAtom, PlaygroundData} from "../../store/playgroundData";
...
const combinePlayground = (playground: PlaygroundData, gameMove: GameMove | null, mark: Mark) => {
    return playground.map((row, rowIndex) => row.map((col, colIndex) => {
...
export const Game:FC<GameProps> = ({ sessionID, account, contract }) => {
    ...
    const playgroundWithUncommited = useMemo(() => combinePlayground(playground, nextMove, Mark.YOUR), [playground, nextMove])
    const [yourTurn, setYourTurn] = useState(false);
    const [gameOver, setGameOver] = useState(false);

    console.log('gameOver', gameOver);

    const commitMove = useCallback((gameMove: GameMove, mark: Mark) => {
        setPlayground((playground) => combinePlayground(playground, gameMove, mark));
    },[setPlayground]);

    const reloadSession = useCallback(() => {
        if(sessionID !== -1){
            getSession(web3, sessionID, account)
                .then((session) => {
    ...
    useEffect(() => {
        ...
        if(sessionID !== -1){
            subscription = contract.events.GameMove({
                // filter: {
    ...
    useEffect(() => {
        ...
        if(sessionID !== -1){
            subscription = contract.events.HasWinner({
                // filter: {
                //     sessionID: sessionID.toString()
                // },
            })
    ...
    const nextMovePossible = useMemo(() => {
        return nextMove !== null && yourTurn && !gameOver && connectionStatus !== 'connected'
    ...
    return (
        <Box className='game'>
            <Box className='game__playground' sx={{
                height:'80%',
                display: 'flex',
                justifyContent: 'center'
            }}>
                <Playground disabled={isPlaygroundDisabled} playground={playgroundWithUncommited} markFigure={markFigure} onMove={handleMakeMove}/>
            </Box>
            <Box className='game__buttons' sx={{
                p: 5,
                textAlign: 'center',
                height:'20%'
            }}>
                <Button variant='contained' type='button' disabled={!nextMovePossible} onClick={handleCommitMove}>
                    {makeMoveButtonText}
                </Button>
            </Box>
        </Box>
